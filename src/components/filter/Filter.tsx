      ...
      <div className=" flex justify-between items-center gap-3 ">
        <div className=" flex flex-row items-center gap-3 ">
          {filterItem?.map((item) => (
            <div
              ...
              className="  flex  items-center gap-1 cursor-pointer border px-6  py-3 rounded-[45px]  border-[#fff]  text-white "
              key={item?.id}
            >
              <h1 className=" md:text-[18px] capitalize  font-medium">
                {" "}
                {item?.name}{" "}
              </h1>
        ...
        <div>
          ...
          <div
            ...
            className=" cursor-pointer  flex  items-center gap-1 border px-6  py-3 rounded-[45px]  border-[#fff]  text-white "
          >
            <h1 className="text-[15xp] md:text-[20px]  font-medium">
              Sort by: {sort?.name}{" "}
            </h1>
            <RiArrowDropDownLine size={18} />
          </div>
          ...
          >
            {/* sort options list  */}
            <div>
              {sortbyProduct?.map((item: any, idnex: number) => (
                <div
                  onClick={() => {
                    setSort(item), setActiveSort(false);
                  }}
                  key={idnex}
        ...
        >
          <div className="    w-[550px] bg-white relative rounded-3xl p-8 ">
            ...
            <div
              ...
              className=" absolute top-1 right-3 p-6 cursor-pointer"
            >
            ...
            <div className=" py-4">
              {activeModal == "colors" && (
                <ColorsComponents
                  colorsData={sizeColorData.colors}
                ></ColorsComponents>
              )}
              {activeModal == "size" && (
                <SizesComponents
                  sizesData={sizeColorData.sizes}
                ></SizesComponents>
              )}
            </div>
          </div>
