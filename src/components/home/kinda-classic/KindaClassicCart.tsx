...
const KindaClassicCart = ({ doubleProduct }: any) => {
    ...
    return (
        <div className=" flex  flex-col gap-5 ">
            <Link href={`/products/${productOne?.id}`}
                onMouseLeave={() => setActive1(false)}
                onMouseOverCapture={() => setActive1(true)}
                className={`w-full ${active1 ? "h-[550px]" : "h-[450px]"} ${active2 ? " !h-[350px]" : "h-[450px]"
            ...
            <Link href={`/products/${productTwo?.id}`}
                onMouseLeave={() => setActive2(false)}
                onMouseOverCapture={() => setActive2(true)}
                className=" w-full !overflow-hidden"
            >
                <div
                    className={`w-full ${active2 ? "h-[550px]" : "h-[450px]"} ${active1 ? "h-[350px]" : "h-[450px]"
